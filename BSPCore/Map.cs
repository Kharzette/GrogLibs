...
public partial class Map
{
	...
	bool FaceContainsPointScore(GFXFace f, Vector3 point, out float offBy)
	{
		...
		for(int i=0;i < f.mNumVerts;i++)
		{
			...
			edgePlane.mNormal	=Vector3.Normalize(edgePlane.mNormal);
	...
	bool RayCollide(Vector3 Front, Vector3 Back, int modelIndex, Matrix4x4 modelInv, out Vector3 impact)
	{
		...
		if(RayIntersect(Front, Back, mGFXModels[0].mRootNode, ref impact, ref hitLeaf))
		{
			return	true;
		}
		...
		if(modelIndex > 0)
		{
			...
			Vector3	frontInv, backInv;
			Mathery.TransformCoordinate(Front, ref modelInv, out frontInv);
			Mathery.TransformCoordinate(Back, ref modelInv, out backInv);
	...
	public Dictionary<int, Matrix4x4> GetModelTransforms()
	{
		...
		{
			Matrix4x4	mat	=Matrix4x4.CreateTranslation(mGFXModels[i].mOrigin);
...
//					}
	...
	public void MakeFullBrightData(MapGrinder mg)
	{
		Vector3	[]vnorms	=MakeSmoothVertNormals();
		mg.BuildFullBrightData(mGFXVerts, mGFXVertIndexes, mGFXRGBVerts, vnorms, mGFXPlanes, mGFXModels);
	}
	...
	public Color[] GetLightMapForFace(int fIdx, out int width, out int height)
	{
		...
		{
			lmap[i]	=new Color(mGFXLightData[face.mLightOfs + (i * 3)],
				mGFXLightData[face.mLightOfs + (i * 3) + 1],
				mGFXLightData[face.mLightOfs + (i * 3) + 2],
				(byte)0xFF);
		}
