...
namespace BSPLib
{
	...
	public class BspNode
	{
		...
		//gather all the node planes that are non leafs
		//filter them through the tree until there is a
		//big list of split up faces based on those node
		//planes.

		//nuke faces created by bsp build splits, but
		//keep the planes

		//pass each down the tree until it lands in a leaf
		//if it lands coplanar to an opposite face, mark
		//it as hidden

		//create big box around world, 6 portals with the
		//front side pointing at outside node, filter them
		//down into the tree, they get split up, eventually
		//point front side to outside, back side to the
		//node they land in
