...
public static class VertexTypes
{
	...
	public static ID3D11Buffer BuildABuffer(ID3D11Device gd, Array verts, int typeIdx)
	{
		if(typeIdx < 0)
		{
			return	null;
	...
	public static ID3D11Buffer BuildAnIndexBuffer(ID3D11Device gd, UInt16 []inds)
	{
		...
		return	gd.CreateBuffer<UInt16>(inds, indDesc);
	}


	public static ID3D11Buffer BuildAnIndexBuffer(ID3D11Device gd, UInt32 []inds)
	{
		if(gd == null || inds == null)
		{
			return	null;
		...
		return	gd.CreateBuffer<UInt32>(inds, indDesc);
	}
		...
		for(int i=0;i < verts.Length;i++)
		{
			foreach(FieldInfo fi in finfo)
			{
				if(fi.Name == "Normal")
				{
					...
					Vector3	norm	=new Vector3((float)vec.X, (float)vec.Y, (float)vec.Z);
		...
		for(int i=0;i < verts.Length;i++)
		{
			foreach(FieldInfo fi in finfo)
			{
				if(fi.Name == "TexCoord" + set)
				{
					...
					Vector2	tex	=new Vector2((float)vec.X, (float)vec.Y);
	...
	public static void ReplaceNormals(Array verts, Vector3 []newNorm)
	{
		...
		for(int i=0;i < verts.Length;i++)
		{
			...
			smallNorm	=new Half4(norm.X, norm.Y, norm.Z, 0f);
		...
		out BoundingBox box, out BoundingSphere sphere)
	{
		...
		sphere	=new BoundingSphere(Vector3.Zero, 0f);
		...
		{
			box		=BoundingBox.CreateFromPoints(boundPoints.ToArray());
		...
		out BoundingBox box, out BoundingSphere sphere)
	{
		...
		box		=BoundingBox.CreateFromPoints(points.ToArray());
